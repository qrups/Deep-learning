...
class OxfordPetDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.transform is not None:
            ...
            transformed["mask"] = transformed["mask"].unsqueeze(0)
        return transformed
...
def load_dataset(data_path, mode):
    ...
    train_transform = A.Compose(
        [
            ...
            A.RandomToneCurve(scale=0.1, per_channel=False, p=0.5),
    ...
    if mode == "train":
        dataset = OxfordPetDataset(root=data_path, mode=mode, transform=train_transform)
    else:
